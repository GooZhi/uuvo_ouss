...
function WR_TSJQ_ZV_VKIH_MACA(IOWR_AFOA) {
    ...
    if (UXUX_YHLD != "object") {
        throw new Error(NVMS_EBWU_LD_YHRJ_1("[MCVN UXUX MSOX , AOAO JI object]" + UXUX_YHLD))
    }
    var BQEO_1 = IOWR_AFOA.WR_AFOA_BQEO.replace(/^\s+|\s+$/g,"");
    var VNWM_MCVN_1 = IOWR_AFOA.WR_AFOA_MCVN.match(/[^,]+=[^,]+/g);
    if (VNWM_MCVN_1 == null) {
        throw new Error(NVMS_EBWU_LD_YHRJ_1("[MCVN NRAP]"))
    }
    var VNAW_1
    var DPHP_1
    var TYUB_1
    VNWM_MCVN_1.forEach(RNSF => {
        switch (true) {
            case /VNAW\b/i.test(RNSF):
                ...
                if (IOWR_VNAW_1 == null) {
                    throw new Error(NVMS_EBWU_LD_YHRJ_1("[MCVN FTPJ]" + RNSF + "<--" + IOWR_AFOA.WR_AFOA_MCVN))
                } else {
            ...
            case /TYUB\b/i.test(RNSF):
                ...
                break;
            default:
                throw new Error(NVMS_EBWU_LD_YHRJ_1("[ACUN MCVN WU]" + RNSF + "<--" + AFOA_UJ.WR_AFOA_MCVN))
        }
    });
